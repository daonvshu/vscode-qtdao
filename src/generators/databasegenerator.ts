...
export class DatabaseGenerator {
    ...
    protected createFieldDeclare(prefix: string): string {
        return this.loadTb.fields
            ...
            .map((field) => `\n${this.tab2}EntityField<${this.getFieldCppType(field.type)}> ${field.name} = EntityField<${this.getFieldCppType(field.type)}>("${this.getFieldNameInDatabase(field.name)}", "${this.createTableName(prefix)}");`)
            .merge();
    }
    ...
    protected createFieldDeclareReset(): string {
        return this.loadTb.fields
            .filter((field) => !field.transient)
            .map((field) => `\n${this.tab3}${field.name} = EntityField<${this.getFieldCppType(field.type)}>("${this.getFieldNameInDatabase(field.name)}", tbName);`)
            .merge();
    }
...
set(ENTITY_FILE_LIST
    $\{CMAKE_CURRENT_LIST_DIR\}/${this.getSqlTypeName()}EntityInclude.h
    $\{CMAKE_CURRENT_LIST_DIR\}/${this.getSqlTypeName()}EntityInclude.cpp
